...
const GreenerAbout = ({ name, about, qualifications, experiences }) => {
  return (
    <div>
      <div>
        ...
        <ul className="grid sm:grid-cols-2 gap-[30px] pt-4 md:p-5">
          {experiences?.map((item, index) => (
            <li key={index} className="p-4 rounded bg-[#fff9ea]">
              ...
              <p className="text-[14px] leading-5 font-medium text-textColor">{item.place}</p>
            </li>
          ))}
        </ul>
      </div>
